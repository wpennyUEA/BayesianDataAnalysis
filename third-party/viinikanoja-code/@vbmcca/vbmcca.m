...
classdef vbmcca
    ...
    methods
        ...
        % initialising a vbmcca object.
            ...
            % obj.phiX = 1/10000*eye(dX); %Inverse scale precision
            % obj.phiY = 1/10000*eye(dY);
        ...
        % obj - Model with initialised parameters.
            ...
            for i = 1 : retries
                ...
                % Reject initialisations with less than 5 points per cluster
                too_sparse=0;
                cc = unique(newIndices);
                for jj=1:length(cc)
                    cc_prior(jj) = length(find(newIndices==1));
                end
                if any(cc_prior<5)
                    too_sparse=1;
                end

                if( distance < objectiveMin & ~too_sparse)
                    centers = newCenters;
                    indices = newIndices;
            ...
            for k=1 : obj.M
                ...
                % C1 = xCov - W1*W1'; C1 = 0.5*(C1+C1');
                % C2 = yCov - W2*W2'; C2 = 0.5*(C2+C2');
                % 
                % obj.eX_psi(:,:,k) = pinv(C1);
                % obj.eY_psi(:,:,k) = pinv(C2);
        ...
        function [obj,energy,F] = updateLatentPosterior(obj,X,Y, nuUpdates, initPhase)
        ...
        %statistics. For internal use.
            ...
            F.name = {'Acc-X1','Acc-X2','KL-Z','KL-U','KL-M'};
            F.term(1) = sum(row_sum(obj.r .* elnX_p));
            F.term(2) = sum(row_sum(obj.r .* elnY_p));
            F.term(3) = sum(row_sum(obj.r .* DKL_t));
            F.term(4) = sum(row_sum(obj.r .* DKL_u));
            F.term(5) = sum(row_sum(DKL_p));
       ...
       % latentStatistics - Inferred latent variable statistics
        ...
        function [obj, energy, F] = updateParameterPosterior(obj,X,Y)
            ...
            % W Penny edits April 2024
            F.name = {'KL-mu1','KL-mu2','KL-Psi1','KL-Psi2','KL-W1','KL-W2','KL-alpha1','KL-alpha2'};
            F.term(1) = sum(DKL_muX);
            F.term(2) = sum(DKL_muY);
            F.term(3) = sum(DKL_psiX);
            F.term(4) = sum(DKL_psiY);
            F.term(5) = sum(row_sum(DKL_WX));
            F.term(6) = sum(row_sum(DKL_WY));
            F.term(7) = sum(row_sum(DKL_alphaX));
            F.term(8) = sum(row_sum(DKL_alphaY));
        end
        
            
        
        function [model, lowerbound, F] = learn(obj, X, Y, parameters)
        ...
        % F - lowerbound broken down into its elements
            ...
            for i = 1:parameters.maxIter
                ...
                [model,penergies(i),Fparam] = model.updateParameterPosterior(X, Y);
                [model,lenergies(i),Flatent] = model.updateLatentPosterior(X, Y, parameters.nuUpdates, i < 10);

                % Edit by Will Penny, to print out progress only every few
                % iterations
                if mod(i,64)==0
                ...
                if( i >= 2 && abs( (penergies(i)+lenergies(i)) - (penergies(i-1)+lenergies(i-1)) ) ...
                    ...
                    disp(strcat('Iteration ', num2str(i-1),', Lower bound: ', num2str(penergies(i-1)+lenergies(i-1))));
                    disp(strcat('Iteration ', num2str(i),', Lower bound: ', num2str(penergies(i)+lenergies(i))));
                ...
                % 
                %     if mod(i,16)==0
                %         disp(sprintf('Iteration %d  deltaW: %1.6f', i, changeW));
                %     end
                % 
            ...
            F.name = [Flatent.name,Fparam.name];
            F.term = [Flatent.term,Fparam.term];
        end
