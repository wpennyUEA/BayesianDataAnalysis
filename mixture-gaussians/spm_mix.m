function [mix] = spm_mix (y,m,options)
% Fit a multivariate Gaussian Mixture model using VB
% FORMAT [mix] = spm_mix (y,m,options)
%
% y          [N x d] data matrix containing N samples of d-dim data
% m          Number of mixture components
% options
%            .maxIter    maximum number of iterations (default 128)
%            .tol        tolerance for convergence (default 10^-4)
%            .verbose    Set to 1 to see evolution of free energy, 0 otherwise
%            (default=1)
%            .retries    Number of retries of initial kmeans
%            .init_mix   Optional initialisation method
%                        .priors, .covs, .means
%                        If not specified spm_mix will call spm_kmeans.m
%
% mix        Returned model
%
%--------------------------------------------------------------------------
...
try retries = options.retries; catch retries=10; end
try maxIter = options.maxIter; catch maxIter = 128; end
try tol = options.tol; catch tol = 10^(-4); end
try init_mix = options.init_mix; catch init_mix = [];end
...
if isempty(init_mix)
    ...
    for r = 1:retries,
        [km(r).priors,km(r).means,km(r).covs] = spm_kmeans(y,m);
        sumd(r) = 0;
        for j=1:m,
            sse = sum(sum((y-km(r).means(j,:)).^2));
            sumd(r) = sumd(r)+sse;
        end
        if verbose, disp(sprintf('Kmeans init: retry %d out of %d, sumd=%1.2f',r,retries,sumd(r))); end
    end
    [tmp,ind] = min(sumd);
    priors = km(ind).priors;
    means = km(ind).means;
    covs = km(ind).covs;
else
    disp('spm_mix: external initialisation')
    priors = init_mix.priors;
    means = init_mix.means;
    covs = init_mix.covs;
end
